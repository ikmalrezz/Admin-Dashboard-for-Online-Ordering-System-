...
Class Actions extends DBConnection{
    ...
    function save_user(){
        ...
        $check = $this->db->query("SELECT count(user_id) as `count` FROM admin where `username` = '{$username}' " . ($id > 0 ? " and user_id != '{$id}' " : ""))->fetch_array()['count'];
        ...
        if(@$check> 0){
            ...
            if(empty($id)){
                $sql = "INSERT INTO `admin` {$data}";
            }else{
                $sql = "UPDATE `admin` set {$data} where user_id = '{$id}'";
            }
    ...
    function delete_user(){
        ...
        @$delete = $this->db->query("DELETE FROM `admin` where user_id = '{$id}'");
        if($delete){
            $resp['status']='success';
    ...
    function update_credentials(){
        ...
        if(!empty($password) && md5($old_password) != $_SESSION['password']){
            ...
            $sql = "UPDATE `admin` set {$data} where user_id = '{$_SESSION['user_id']}'";
            @$save = $this->db->query($sql);
            if($save){
                $resp['status'] = 'success';
                $_SESSION['flashdata']['type'] = 'success';
